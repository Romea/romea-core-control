...
//-----------------------------------------------------------------------------
FollowMe::FollowMe(const double & sampling_period,
...
//-----------------------------------------------------------------------------
double FollowMe::computeAngularSpeed(const double & lateral_deviation,
...
//-----------------------------------------------------------------------------
FrontRearData FollowMe::computeSteeringAngles(const double & wheelbase,
                                              ...
                                              const double & desired_longitudinal_distance)
{
  ...
  if (std::fabs(vitesse)>VitMin_)
  {
    ...
    front_steering_angle_command  = std::atan(wheelbase*(kd_*EpsThet*std::cos(0*Thet2)+CoefYawrate*yaw_rate_leader)/(vitesse*std::cos(rear_steering_angle)) + std::tan(rear_steering_angle));
  }

  if(std::abs(front_steering_angle_command) > maximal_front_steering_angle)
  {
    front_steering_angle_command = copysign(maximal_front_steering_angle, front_steering_angle_command);
  }
  ...
  double rear_steering_angle_command=-course_deviation-(1/kd_)*( kdd_ *ThetaError2 - kd_*omega_d);
                                              ...
                                              double &theta_consigne)
{
  ...
  std::cout << " lateral deviation +yd  "<<lateral_deviation+1.35 <<" lateral deviation -yd  "<<lateral_deviation-1.35 <<" desired_lat_dev_ "<< desired_lat_dev_<< std::endl;
  ...
  double braq_F  = std::atan((kd_*EpsThet+curvature)*wheelbase*std::cos(Thet2)/cos(rear_steering_angle+rear_sliding_angle)
  ...
  double braq_R=-course_deviation-rear_sliding_angle-(1/kd_)*( kdd_*ThetaError2 - kd_*omega_d);

  std::cout<<" new command "<<omega_d <<" "<< Thet2<<" "<< EpsThet<<" "<< braq_F<<" "<< theta_consigne<<" "<<ThetaError2<<" "<<braq_R << std::endl;
                                              ...
                                              double &theta_consigne)
{
  ...
  double commande_inter = ((-lateral_deviation-0*courbe0)*Sigma + lateral_deviation*SigmaExp - 0*courbe2*Sigma3)/Sigma2  + 1*courbe1;
  //    std::cout << " ******************************* commande_inter = " << commande_inter << "A =  " << courbe1 << std::endl;
  if (commande_inter>0.9)
    commande_inter=std::copysign(0.9,commande_inter);
  ...
  omega_d = lambda*omega_d + (1-lambda)*atan (kp_*(lateral_deviation-desired_lat_dev_)/alpha);
  ...
  double braq_F  = atan((kd_*EpsThet+ (curvature))*wheelbase*cos(Thet2)/cos(rear_steering_angle+rear_sliding_angle) + tan(rear_steering_angle+rear_sliding_angle)) - front_sliding_angle;
